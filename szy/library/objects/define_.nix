{ identifier, lib, utils, importLib, helper, qualifiers, internal, ... }@gInputs:
rec {
	...
	in
	{
		...
		[
			(
				{
					"${identifier}"."test"."${template}+${name}" = utils.options.createFromKeys { keys = gTemplate.meta.namespace; value =
					let
										...
										module =
										{
											...
											options =
											{
												...
												full.extends = utils.options.constant 
												{
													...
													value =
													let
														templates = [ gTemplate ] ++ (builtins.map (name: helper.getTemplate { inherit (inputs) config name; }) final.meta.extends);
					...
					{
						type = internal.mkNestedModule definitionModule;
						default = {};
					}; };
				}
			)
