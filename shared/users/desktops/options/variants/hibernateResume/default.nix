...
{ szy, lib, config, pkgs, ... }:
{
	...
	options."${szy}".desktops.options.hibernateResume = {
	...
	in
	lib.mkIf (variant.enabled && ((builtins.length config."${szy}".desktops.options.hibernateResume.commands) != 0))
	{
		...
		''
      		if [ "$1-$SYSTEMD_SLEEP_ACTION" = "post-hibernate" ]; then
				${lib.strings.concatLines config."${szy}".desktops.options.hibernateResume.commands}
      		fi
    	'';
