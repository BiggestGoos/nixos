...
in
{
  ...
  config.wayland.windowManager.hyprland.settings = {
    ...
    bindir = lib.lists.optionals (desktop.isEnabledStrict [ "hyprland" ]) [
    ...
    bind =
      ...
      (lib.lists.optionals (desktop.isEnabledStrict [ "hyprland" ]) (
        [
          ...
          "${mainMod}, E, exec, ${launchPrefix} ${
            (applications.default.fileManager.gui.commands or { open = ""; }).open
          }"
        ...
        ++ (builtins.map (i: "${mainMod}, ${builtins.toString i}, workspace, ${builtins.toString i}") (
          builtins.genList (x: x) 10
        ))
        ++ (builtins.map (
          i: "${mainMod} + SHIFT, ${builtins.toString i}, movetoworkspace, ${builtins.toString i}"
        ) (builtins.genList (x: x) 10))
      ))
      ++ [
