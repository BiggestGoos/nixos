...
let
  ...
  devices = osConfig."${szy}".desktops.components.devices;
  ...
  genericPointerConfig = genericPointer: {
    ...
    accel_profile =
      let
        profile =
          if (builtins.isString genericPointer.acceleration) then
            genericPointer.acceleration
          else
            "custom ${builtins.toString genericPointer.acceleration.movement.step} ${
              lib.strings.concatStringsSep " " (
                builtins.map (point: builtins.toString point) genericPointer.acceleration.movement.points
              )
            }";
      in
      lib.mkIf (genericPointer.acceleration != null && genericPointer.acceleration != "none") profile;

    force_no_accel = lib.mkIf (
      genericPointer.acceleration != null && genericPointer.acceleration == "none"
    ) true;

    scroll_points =
      let
        scrollPoints = "${builtins.toString genericPointer.acceleration.scrolling.step} ${
          lib.strings.concatStringsSep " " (
            builtins.map (point: builtins.toString point) genericPointer.acceleration.scrolling.points
          )
        }";
      in
      lib.mkIf (
        (genericPointer.acceleration != null)
        && ((builtins.isString genericPointer.acceleration) == false)
        && (genericPointer.acceleration.scrolling != null)
      ) scrollPoints;
    ...
    natural_scroll = lib.mkIf (
      genericPointer.scrolling.natural != null
    ) genericPointer.scrolling.natural;
  ...
  touchpadConfig = touchpad: {
    ...
    middle_button_emulation = lib.mkIf (
      touchpad.middleButtonEmulation != null
    ) touchpad.middleButtonEmulation;
    ...
    tap-and-drag = lib.mkIf (touchpad.tapAndDrag != null) (
      if (builtins.isBool touchpad.tapAndDrag) then touchpad.tapAndDrag else true
    );
    drag_lock =
      lib.mkIf (touchpad.tapAndDrag != null && (builtins.isString touchpad.tapAndDrag))
        ({
          noLock = 0;
          timeout = 1;
          sticky = 2;
  ...
  wayland.windowManager.hyprland.settings = {
    ...
    device =
      ...
      (lib.attrsets.mapAttrsToList (
        ...
        szy.utils.mergeAll [
          (keyboardConfig keyboard)
          { name = keyboard.hardware.name; }
        ]
      ) keyboards)
      ++ (lib.attrsets.mapAttrsToList (
        name: mouse:
        szy.utils.mergeAll [
          (genericPointerConfig mouse)
          (mouseConfig mouse)
          { name = mouse.hardware.name; }
        ]
      ) mice)
      ++ (lib.attrsets.mapAttrsToList (
        name: touchpad:
        szy.utils.mergeAll [
          (genericPointerConfig touchpad)
          (touchpadConfig touchpad)
          { name = touchpad.hardware.name; }
        ]
      ) touchpads)
      ++ (lib.attrsets.mapAttrsToList (
        name: genericPointer:
        szy.utils.mergeAll [
          (genericPointerConfig genericPointer)
          { name = genericPointer.hardware.name; }
        ]
      ) genericPointers);
