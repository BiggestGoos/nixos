...
let
  ...
  hosts' = lib.attrsets.filterAttrs (name: value: !(lib.strings.hasPrefix "__" name)) root.hosts;
  ...
  hosts = lib.attrsets.mapAttrs (
    ...
    let
      ...
      flakeFiles = (sharedFlakeFiles) ++ (getFlakeFiles value);
    in
    {
      inherit (metaData) root;
      inputs = szy.lib.attrsets.deepMergeList (
        builtins.map (
          file:
          let
            input' = import file;
            input = if (builtins.isFunction input') then input' szy else input';
          in
          input
        ) flakeFiles
      );
      partitioning =
        let
          ...
          set = szy.lib.attrsets.deepMergeList (builtins.map (file: import file) diskoFiles);
        ...
        if (set != { }) then
          {
            ...
            disks = lib.attrsets.mapAttrsToList (
              name: value: "partitionDisks[\"${value.device}\"]='${builtins.toJSON { "${name}" = value; }}'"
  ...
  setups = lib.attrsets.mapAttrsToList (name: value: ''
    if [[ "$hostname" == "${name}" ]]
    then
