...
}:
{
  ...
  options."${szy}".desktops.components.displays =
    let
      displayNames = (builtins.attrNames (config."${szy}".desktops.components.displays.displays));
    in
    {
        ...
        type =
          let
            template.options = {
                ...
                type =
                  ...
                  lib.types.either
                    (lib.types.enum (
                      ...
                      ++ (builtins.map (direction: "auto-center-${direction}") directions)
                    ))
                    (
                      lib.types.submoduleWith {
                        modules = [
              ...
              scale = lib.mkOption {
                type = lib.types.either (lib.types.addCheck lib.types.number (x: x >= 0.0)) (
                  lib.types.enum [ "auto" ]
                );
                default = "auto";
              };
      ...
      default = {
        ...
        values = lib.mkOption {
          ...
          readOnly = true;
          default =
            config."${szy}".desktops.components.displays.displays."${config."${
              szy
            }".desktops.components.displays.default.name
            }";
        };
