  ...
  setDefault =
    ...
    lib.attrsets.mapAttrs (
      ...
      if (!isOverride) then
        ...
        (types."${value.override}" value.value)
      else
        (lib.mkOverride value.override value.value)
    ) variables;
...
in
{
  ...
  options."${szy}".variables = lib.options.mkOption {
    type =
      ...
      in
      lib.types.attrsOf (
        lib.types.either lib.types.str (lib.types.submoduleWith { modules = [ overrideModule ]; })
      );
  };
