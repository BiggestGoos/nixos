...
szy.objects.declare {
                ...
                { config, ... }:
                {
                  options = {
                    ...
                    finalName = lib.options.mkOption {
                      ...
                      readOnly = true;
                      default =
                        if (config.path == null) then null else (builtins.head (builtins.match ".*/([^/]*)" config.path));
                    };
  ...
  defaultArguments =
    { final, template }:
    {
      ...
      program.bin.default.name =
        lib.mkDefault
          (final.data.package or { meta = { }; }).meta.mainProgram or null;
      ...
      commands =
        let
          ...
          arguments = lib.attrsets.filterAttrs (name: value: value.generateCommand) rawArguments;
