  ...
  options."${szy}".applications =
    let
      ...
      getDefinitions =
        template:
        let
          meta = szy.objects.helper.template.getMeta {
            inherit config;
            identifier = template;
          };
          all = builtins.map (
        ...
        in
        builtins.filter (definition: definition.data.enabled) all;
            ...
            builtins.map (template: {
              ...
              value =
                ...
                lib.attrsets.mapAttrs (
                  name: value: if (value.value == null) then null else inheritApplication value.value
                ) defaults;
            }) defaultTemplates
          );
      ...
      modules = builtins.map (template: {
        ...
        options."${template.meta.name}" = lib.options.mkOption {
          ...
          default =
            ...
            in
            lib.attrsets.mapAttrs (name: value: inheritApplication value) definitions;
        };
