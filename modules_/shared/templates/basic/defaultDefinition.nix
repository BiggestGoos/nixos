    ...
    { final }:
    {
      ...
      default =
        let
          ...
          gDefinitions = builtins.filter (
            identifier: (szy.objects.helper.definition.get ({ inherit config identifier; })).data.enabled
          ) definitionsBase;
          gAllDefinitions = builtins.filter (
            identifier: (szy.objects.helper.definition.get ({ inherit config identifier; })).data.enabled
          ) allDefinitionsBase;
          ...
          base =
            ...
            let
              ...
              definitions = builtins.filter filterFunc (
                builtins.map (
                  identifier: szy.objects.helper.definition.get { inherit config identifier; }
                ) gDefinitions
              );
              allDefinitions = builtins.filter filterFunc (
                builtins.map (
                  identifier: szy.objects.helper.definition.get { inherit config identifier; }
                ) gAllDefinitions
              );
                  ...
                  let
                    ...
                    module.options = {
                      ...
                      template = lib.options.mkOption {
                        type = lib.types.enum (
                          builtins.map (definition: definition.meta.identifier.template) allDefinitions
                        );
                        default =
                          let
                            inherit (identifier) name;

                            possibleDefinitions = builtins.filter (
                              definition: definition.meta.identifier.name == name
                            ) allDefinitions;
