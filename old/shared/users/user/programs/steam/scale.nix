  ...
  home.activation.setSteamScale =
    let
      ...
      scale =
        if
          (
            osConfig."${szy}".desktops.desktopData != null
            && osConfig."${szy}".desktops.desktopData.isDefault [ "hyprland" ]
          )
        then
          builtins.toString osConfig."${szy}".desktops.components.displays.default.values.scale
        else
          "1.0";
    ...
    lib.hm.dag.entryAfter [ "writeBoundary" ] ''
      run ${
        (pkgs.writers.writePython3 "setSteamScale" { libraries = [ pkgs.python313Packages.vdf ]; }
