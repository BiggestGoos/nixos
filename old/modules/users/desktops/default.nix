...
in
{
  ...
  options."${szy}".desktops = {
    ...
    user.import = lib.mkOption {
      ...
      default =
        path:
        if (config."${szy}".desktops.enabled == null) then
          [ ]
        else
          (lib.lists.flatten (
            builtins.map (
              ...
              let
                ...
                paths = lib.lists.remove null (
                  lib.lists.imap1 (
                    i: e:
                    let
                      resolvedPath = (
                        (path + "/${(lib.strings.concatStringsSep "+" (lib.lists.take i enabledDesktop.names))}")
                      );
                    in
                    (if (builtins.pathExists resolvedPath) then resolvedPath else null)
                  ) enabledDesktop.names
                );
              in
              paths
    ...
    available = lib.mkOption {
      ...
      default = builtins.attrNames (
        builtins.removeAttrs (lib.attrsets.mapAttrs' (
          name: value:
          (
        ...
        ) (config."${szy}".desktops.desktops or { })) [ "" ]
      );
    };
  ...
  config."${szy}".profiles.base.branches = (
    lib.attrsets.setAttrByPath resolvedProfilePrefix {
          ...
          builtins.map (
            ...
            let
              compareLists =
                l1: l2:
                (
                  if ((builtins.length l1) > (builtins.length l2)) then
                    false
                  else
                    (lib.lists.all (x: x == true) (lib.lists.imap0 (i: e1: (e1 == (builtins.elemAt l2 i))) l1))
                );
              compareListsStrict = l1: l2: l1 == l2;

              availableDesktopsNames = builtins.map (
                desktopName: config."${szy}".desktops.desktops."${desktopName}".names
              ) availableDesktops;

              enabledDesktopsNames = builtins.map (
                desktopName: config."${szy}".desktops.desktops."${desktopName}".names
              ...
              assertDesktop =
                names:
                assert (
                  lib.lists.any (x: x == true) (
                    builtins.map (desktopNames: compareLists names desktopNames) availableDesktopsNames
                  )
                );
                names;
              desktopData = rec {
                ...
                isEnabled =
                  names:
                  (lib.lists.any (x: x == true) (
                    builtins.map (desktopNames: compareLists (assertDesktop names) desktopNames) enabledDesktopsNames
                  ));
                ...
                isEnabledStrict =
                  ...
                  (lib.lists.any (x: x == true) (
                    builtins.map (
                      desktopNames: compareListsStrict (assertDesktop names) desktopNames
                    ) enabledDesktopsNames
                  ));
              };
            in
            {
              ...
              value = {
                configuration = {
                  ...
                  imports =
                    (lib.lists.flatten (
                      builtins.map (
                        currentDesktop: (config."${szy}".desktops.desktops."${currentDesktop}".imports)
                      ) desktopValues.enabled
                    ))
                    ++ [ ({ _module.args.desktop = desktopData; }) ]
                    ++ (builtins.map (
                      currentDesktop: (config."${szy}".desktops.desktops."${currentDesktop}".configuration)
                    ) desktopValues.enabled);
