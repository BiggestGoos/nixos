    ...
    options = {
      ...
      final =
        let
          ...
          resolveOverrides = overrides: lib.attrsets.filterAttrs (name: value: value != null) overrides;
          ...
          values =
            let
              ...
              fullActions = fullSet (base.values.actions or { }) (defaultOverrides.actions or { }) (
                overrides.actions or { }
              );
              fullExtra = fullSet (base.values.extraConfig or { }) (defaultOverrides.extraConfig or { }) (
                overrides.extraConfig or { }
              );
            in
            baseValues
            // {
          ...
          notComplete = lib.lists.any (value: value == false) (
            builtins.map (key: builtins.elem key (builtins.attrNames values)) requiredKeys
          );

          result =
            if (notComplete) then
              (lib.trivial.throwIf required "The required desktop entry for definition \"${final.meta.name}\" of template \"${final.meta.template}\" could not be created. It is missing either name or desktopName.") null
            else
              "${desktopEntryPackage}/share/applications/${values.name}.desktop";
        ...
        in
        {
          ...
          id = lib.options.mkOption {
            ...
            readOnly = true;
            default =
              if (notComplete) then null else "${builtins.replaceStrings [ "/" ] [ "-" ] values.name}.desktop";
          };
        ...
        values = lib.options.mkOption {
          ...
          default =
            let
              ...
              path =
                if (lib.strings.hasInfix "/" base) then
                  base
                else
                  "${packagePath}/share/applications/${base}.desktop";
              ...
              fileNameFromShare = lib.strings.removeSuffix ".desktop" (
                lib.strings.concatStrings (
                  builtins.map (part: if (builtins.isList part) then fileNameSplitter else part) splitFileName
                )
              );
              fileNameFromLast = builtins.head (builtins.match ".*/([^/]*).desktop" path);

              fileName =
                if (lib.strings.hasInfix fileNameSplitter path) then fileNameFromShare else fileNameFromLast;
              ...
              actionNames = builtins.filter (value: value != null) (
                builtins.map (row: if !(builtins.isList row) then null else builtins.head row) splitRaw
              );
              ...
              rawActionSplits = lib.lists.drop (if ((builtins.length actionNames) == 1) then 0 else 1) (
                lib.lists.foldl (
                  list: action:
                  let
                    ...
                    data = if (isFirst) then raw else lib.lists.last (lib.lists.last list);
                ...
                builtins.listToAttrs (
                  builtins.map (
                    ...
                    let
                      ...
                      value =
                        if (i != end) then
                          builtins.head (builtins.elemAt rawActionSplits i)
                        else
                          lib.lists.last (lib.lists.last rawActionSplits);
                    in
                    {
                      name = action;
              ...
              parseRows =
                ...
                let
                  ...
                  removeExtra = lib.attrsets.filterAttrs (name: value: (builtins.elem name names)) keyValue;

                  extra = lib.attrsets.filterAttrs (name: value: !(builtins.elem name names)) keyValue;

                  mappedKeys = lib.attrsets.mapAttrs' (
                    name: value:
                    let
                    ...
                    in
                    {
                      ...
                      value =
                        ...
                        in
                        if (builtins.elem mappedName keyIsList) then
                          result
                        else if (builtins.elem mappedName keyIsBool) then
              ...
              actions = lib.attrsets.mapAttrs (name: value: parseRows true value) actionsRows;
...
in
{
  ...
  desktopEntry = lib.options.mkOption {
    ...
    type = lib.types.nullOr (
      lib.types.attrsOf (lib.types.submoduleWith { modules = [ desktopEntry ]; })
    );
