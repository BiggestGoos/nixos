    ...
    }:
    {
      ...
      __functor =
        ...
        let
          ...
          evaluatedComponents = lib.attrsets.mapAttrs (name: value: {
            enable = lib.mkDefault value.enable; # TODO: Look into if this really can be lib.mkDefault'ed
            path =
              if (builtins.isPath value.path) then
                value.path
              else
                (
                  (lib.trivial.throwIfNot (builtins.isPath componentPath)
                    "Must set componentPath to a path value or use only real paths for components."
                    (componentPath)
                  )
            ...
            imports =
              let
                ...
                toggledComponents = lib.attrsets.mapAttrsToList (
                  ...
                  in
                  lib.lists.last (moduleInput.importLib.mkToggleable enabled (lib.lists.toList value.path))
                ) evaluatedComponents;
