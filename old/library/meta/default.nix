...
let
  ...
  compareTypes =
    ...
    if (lType == rType) then
      ...
      (if ((nestRemover lType) == (nestRemover rType)) then true else false);
    ...
    rec {
      ...
      typeCheckData =
        ...
        lib.attrsets.filterAttrs (
          ...
          let
            result =
              (lib.asserts.assertMsg (builtins.hasAttr name dataValues) "callerData value \"${name}\" does not exist.")
              && (
                if (name == "config") then
                  true
                # There is an infinite recursion if we try to check the type of config
                else
                  (lib.asserts.assertMsg (compareTypes dataValues."${name}" (typeOf value))
      ...
      testData =
        ...
        if
          (lib.lists.all (
            value:
            lib.asserts.assertMsg (builtins.elem value (builtins.attrNames dataValues)) "The required field \"${value}\" is not a real field."
          ) requiredFields)
        then
          (lib.lists.naturalSort (
            lib.lists.intersectLists (requiredFields) (builtins.attrNames resolvedData)
          )) == (lib.lists.naturalSort (requiredFields))
        else
          false;
      ...
      assertData =
        ...
        lib.asserts.assertMsg (testData inputs) "The callerData { ${builtins.toJSON data} } did not contain all the required fields { ${builtins.toJSON requiredFields} }.";
